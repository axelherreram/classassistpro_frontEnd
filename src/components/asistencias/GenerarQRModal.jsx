...
import { X, QrCode, Clock, Copy, Check, Users, RefreshCw } from 'lucide-react';
...
export default function GenerarQRModal({ isOpen, onClose, sesionId }) {
  ...
  const handleRegenerateQR = async () => {
    ...
    try {
      const data = await asistenciaService.generarQR(sesionId, true);
      setQrData(data);
      toast.success('Código QR regenerado con éxito (modo tarde)', { icon: '🔄' });
    } catch (err) {
            ...
            <div className="flex flex-col items-center">
              ...
              <div className="flex items-center gap-2 mb-6">
                ...
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-amber-600 hover:bg-amber-100 rounded-lg transition-colors border border-amber-200 disabled:opacity-50"
                  title="Regenerar QR y marcar próximas asistencias como Tarde"
                >
                  <RefreshCw className={`w-4 h-4 ${isRegenerating ? 'animate-spin' : ''}`} />
                  Regenerar (Modo Tarde)
                </button>
              </div>
